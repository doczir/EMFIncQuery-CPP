...
import org.eclipse.incquery.runtime.localsearch.planner.PlanState;
...
public class CPPLocalSearchRuntimeBasedStrategy {

	private boolean allowInverseNavigation;

	public CPPLocalSearchRuntimeBasedStrategy() {
		this(true);
	}

	public CPPLocalSearchRuntimeBasedStrategy(final boolean allowInverseNavigation) {
		this.allowInverseNavigation = allowInverseNavigation;
	}

	/**
	 * The implementation of a local search-based algorithm to create a search
	 * plan for a flattened (and normalized) PBody
	 * 
	 * @param pBody
	 *            for which the plan is to be created
	 * @param logger
	 *            that logs the happenings
	 * @param initialBoundVariables
	 *            variables that are known to have already assigned values
	 * @param metaContext
	 *            the metamodel related information
	 * @param runtimeContext
	 *            the instance model related information
	 * @param hints
	 *            the optional hints for the plan creation
	 * @return the complete search plan for the given {@link PBody}
	 */
	public SubPlan plan(PBody pBody, Logger logger, Set<PVariable> initialBoundVariables, IQueryMetaContext metaContext,
			IQueryRuntimeContext runtimeContext, Map<String, Object> hints) {

		// 1. INITIALIZATION
		// Create a starting plan
		SubPlanFactory subPlanFactory = new SubPlanFactory(pBody);

		// We assume that the adornment (now the bound variables) is previously
		// set
		...
		// The characteristic function tells whether a given adornment is
		// backward reachable from the (B)* state, where
		// each variable is bound.
		// The characteristic function is represented as a set of set of
		// variables
		// TODO this calculation is not not implemented yet, thus the contents
		// of the returned set is not considered later
			...
			for (int j = 0; j < k && j < stateTable.get(i).size(); j++) {
				...
				for (PConstraintInfo constraintInfo : currentState.getPresentExtends()) {
					...
					if (currentState.getBoundVariables().size() == newState.getBoundVariables().size()) {
						// This means no variable binding was done, go on with
						// the next constraint info
						continue;
					}
	...
	private boolean checkInsertCondition(List<PlanState> list, PlanState newState,
		...
		// boolean isAmongBestK = (a == (k + 1)) && c < a &&
		// reachableBoundVariableSets.contains(newState.getBoundVariables());
		...
		for (int j = 0; j < k; j++) {
			if (j < stateTable.get(i2).size()) {
				...
				if (newState.getBoundVariables().equals(stateInTable.getBoundVariables())) {
					// The new state has the same adornment as the stored one -
					// they are not adornment disjoint
					a = j;
				}
	...
	private List<Set<PVariable>> reachabilityAnalysis(PBody pBody, List<PConstraintInfo> constraintInfos) {
		// TODO implement reachability analisys, also save/persist the results
		// somewhere
	 ...
	 * @param runtimeContext
	 *            the model dependent runtime contest
	 * @return a collection of the wrapper PConstraintInfo objects with all the
	 *         allowed application conditions
	 */
	private List<PConstraintInfo> createPConstraintInfos(Set<PConstraint> constraintSet,
		...
		for (PConstraint pConstraint : constraintSet) {
			if (pConstraint instanceof TypeConstraint) {
				...
				doCreateConstraintInfosForTypeConstraint(runtimeContext, constraintInfos, (TypeConstraint) pConstraint,
						affectedVariables, bindings);
			} else {
				// Create constraint infos so that only single use variables can
				// be unbound
				Set<PVariable> affectedVariables = pConstraint.getAffectedVariables();
				...
				// Generate bindings by taking the unioning each element of the
				// power set with the set of non-single use variables
				...
				if (pConstraint instanceof PatternMatchCounter) {
					// in cases of this type, the deduced variables will contain
					// only the result variable
	...
	private void doCreateConstraintInfosForTypeConstraint(IQueryRuntimeContext runtimeContext,
		...
		if (!allowInverseNavigation) {
			// When inverse navigation is not allowed, filter out operation
			// masks, where
			// the first variable would be free AND the feature is an EReference
			// and has no EOpposite
			bindings = excludeInverseNavigationOperationMasks(typeConstraint, bindings);
		} else {
			// Also do the above case, if it is an EReference with no EOpposite,
			// or is an EAttribute
	...
	private Set<Set<PVariable>> excludeInverseNavigationOperationMasks(TypeConstraint typeConstraint,
			Set<Set<PVariable>> bindings) {
	...
	private void doCreateConstraintInfos(IQueryRuntimeContext runtimeContext, List<PConstraintInfo> constraintInfos,
		...
		for (Set<PVariable> boundVariables : bindings) {
			PConstraintInfo info = new CPPPConstraintInfo(pConstraint, boundVariables,

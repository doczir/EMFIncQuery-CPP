...
possible to generate \Java{} code and facilities from a modell specifiaction
...
\EMF{} uses \emph{ECore} as it's main modeling language, which is a partial but most commonly aknowledged implementation of the OMG's
...
On the image it is clearly visible that the concepts metamodel and instancemodel
...
\begin{itemize}
  ...
  \item \emph{EAttribute} - Describes an attribbute which has a name and a type.
  For example an EClass can have an attribbute \emph{Name} which has a type of
  \emph{String}.  
...
reflective API, whith which it is possible to generically access the data of an
...
courses, while a course can only be tought to a single class. Each course has a
...
The root of the model is a school instance with the name Budapest Univesity of
...
Image \figref{InstanceModel_Eclipse} shows the same instancemodel, but as seen
...
internal parameters to the local ones. The thirs pattern,
...
language. The API consits of classes provided by \EIQ{} and the generated code.
